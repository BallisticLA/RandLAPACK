    ...
    static void test_CholQRCP1_approx_qual(int64_t m, int64_t n, int64_t k, int64_t d, int64_t nnz, T tol, std::tuple<int, T, bool> mat_type, uint32_t seed, int test_num) {
        ...
        switch (test_num)
        {
            case 1:
            {
                ...
                std::ofstream ofs;
                ofs.open("../../../testing/test_benchmark/QR/accuracy/raw_data/R_R_norm_ratio_m_"       + std::to_string(m) 
                                                                                       + "_n_"          + std::to_string(n) 
                                                                                       + "_k_"          + std::to_string(k) 
                                                                                       + "_d_"          + std::to_string(d) 
                ...
                // Open a new file
                std::fstream file("../../../testing/test_benchmark/QR/accuracy/raw_data/R_R_norm_ratio_m_"     + std::to_string(m) 
                                                                                              + "_n_"          + std::to_string(n) 
                                                                                              + "_k_"          + std::to_string(k) 
                                                                                              + "_d_"          + std::to_string(d) 
                                                                                              + "_log10(tol)_" + std::to_string(int(log10(tol)))
            ...
            case 2:
            {
                ...
                // Clear the file if it exists
                std::ofstream ofs;
                ofs.open("../../../testing/test_benchmark/QR/accuracy/raw_data/r_s_ratio_m_"   + std::to_string(m) 
                                                                                       + "_n_"          + std::to_string(n) 
                                                                                       + "_k_"          + std::to_string(k) 
                                                                                       + "_d_"          + std::to_string(d) 
                                                                                       + "_log10(tol)_" + std::to_string(int(log10(tol)))
                ...
                // Open a new file
                std::fstream file("../../../testing/test_benchmark/QR/accuracy/raw_data/r_s_ratio_m_"   + std::to_string(m) 
                                                                                       + "_n_"          + std::to_string(n) 
                                                                                       + "_k_"          + std::to_string(k) 
                                                                                       + "_d_"          + std::to_string(d) 
                                                                                       + "_log10(tol)_" + std::to_string(int(log10(tol)))
...
int main(int argc, char **argv){
    ...
    test_CholQRCP1_approx_qual<double>(13107, 200, 200, 200, 1, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2500, false), 1, 1);
    return 0;
}
