...
// Format conversion utilities are now available via the RandLAPACK_demos namespace
...
int main(int argc, char *argv[]) {
    ...
    auto input_mat_coo = RandLAPACK_demos::coo_from_matrix_market<double>(std::string(argv[2]));
    ...
    auto input_mat_csc = RandLAPACK_demos::coo_submatrix_to_csc<double>(m, n, input_mat_coo);

    // Allocate basic workspace.
    ABRIK_benchmark_data<double, RandBLAS::sparse_data::csc::CSCMatrix<double>> all_data(input_mat_csc, m, n, tol);
    all_data.A_input = &input_mat_csc;
    // Read matrix into spectra format
    //RandLAPACK_demos::eigen_sparse_from_matrix_market<double>(std::string(argv[2]), all_data.A_spectra);
    RandLAPACK_demos::eigen_sparse_submatrix_from_matrix_market<double>(std::string(argv[2]), all_data.A_spectra, submatrix_dim_ratio);
