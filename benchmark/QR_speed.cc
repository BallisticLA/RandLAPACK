...
test_speed_mean(int r_pow, int r_pow_max, int col, int col_max, int runs, int nnz, int num_threads)
{
    ...
    for(; r_pow <= r_pow_max; ++r_pow)
    {
        ...
        for (; cols <= col_max; cols += 64)
        {
            ...
            printf("Average timing of CholQRCP for %d runs: %52f μs.\n",                                runs - 1, cholqrcp_avg);
            printf("Average timing Householder vector restoration for %d runs: %34f μs.\n",             runs - 1, rest_avg);
            printf("Average timing of GEQP3 for %d runs: %55f μs.\n",                                   runs - 1, geqp3_avg);
            printf("Average timing of GEQRF for %d runs: %55f μs.\n",                                   runs - 1, geqrf_avg);
            printf("Average timing of GEQR for %d runs: %54f μs.\n",                                    runs - 1, geqr_avg);
            printf("Average timing of TSQRP for %d runs: %55f μs.\n\n",                                 runs - 1, tsqrp_avg);
