// CQRRTO_linops scaling study - measures performance and accuracy as matrix size varies
// Tests both CQRRTO (preconditioned) and CholQR (unpreconditioned) on tall sparse matrices
// Supports two modes:
...
// Shared helpers (cqrrto_bench_common.hh): env-knob provenance, the
...
static std::vector<scaling_result<T>> run_algorithms(
    ...
    // its OWN single timed call() below, all with test_mode=false. CQRRTO_linop
    ...
    // ============================================================
    // Run CQRRTO (preconditioned Cholesky QR) - multiple runs
    // ============================================================
    {
        T* R_cqrrto = new T[n * n];
        for (int64_t run = 0; run < num_runs; ++run) {
            std::fill(R_cqrrto, R_cqrrto + n * n, (T)0);
            auto state_copy = run_states[run];  // Per-run RNG state

            RandLAPACK::CQRRTO_linops<T, RNG> CQRRTO_QR(true, tol, false);  // timing=true, test_mode=false
            CQRRTO_QR.nnz = sketch_nnz;
            CQRRTO_QR.block_size = block_size;

            RandLAPACK::PeakRSSTracker cqrrto_mem;
            cqrrto_mem.start();
            int status = CQRRTO_QR.call(A_linop, R_cqrrto, n, d_factor, state_copy);
            results[run].cqrrto.peak_rss_kb = cqrrto_mem.stop();
            results[run].cqrrto.qr_status = status;

            if (status == 0) {
                results[run].cqrrto.time = CQRRTO_QR.total_us();
                results[run].cqrrto.breakdown.assign(CQRRTO_QR.times.begin(), CQRRTO_QR.times.end() - 1);
                results[run].cqrrto.chol_retries = CQRRTO_QR.n_chol_retries;
                fold_chol_shift(results[run].cqrrto.chol_shift_abs, results[run].cqrrto.chol_shift_rel,
                                 CQRRTO_QR.chol_applied_shifts, CQRRTO_QR.chol_gram_traces, 1);

                // Uniform Q computation for every run: Q = A * R^{-1} via operator
                compute_Q_from_R(A_linop, R_cqrrto, n, Q_uniform, m, n);
                results[run].cqrrto.orth_error    = RandLAPACK::testing::orthogonality_error<T>(Q_uniform, m, n);
                results[run].cqrrto.is_orthonormal = (results[run].cqrrto.orth_error <= std::pow(std::numeric_limits<T>::epsilon(), (T)0.75));
                results[run].cqrrto.max_orth_cols  = RandLAPACK::testing::max_orthonormal_cols<T>(Q_uniform, m, n);
            } else {
                // Failure: no valid R, so no Q and no orthogonality metrics.
                // Full-width -1 breakdown (never a truncated/empty vector).
                results[run].cqrrto.breakdown.assign(10, -1L);
                std::cerr << "Warning: CQRRTO_linop call() failed (run=" << run
    ...
    if (!skip_dense) {
        ...
        for (int64_t run = 0; run < num_runs; ++run) {
            ...
            // Uses same per-run RNG state as CQRRTO_linop for fair comparison
            std::fill(R_dense, R_dense + n * n, (T)0);
            auto state_copy = run_states[run];  // Same RNG state as CQRRTO_linop's run
            ...
            if (status == 0) {
                ...
                // Breakdown matches linop CQRRTO layout: materialize, saso, qr, trtri(=0), precond, gram, trmm_gram(=0), potrf, finalize, rest
                ...
                // Dense CQRRT (rl_cqrrt.hh CQRRT, not CQRRTO_linops) has
                ...
                // same as CQRRTO_linops's own single-pass record).
        ...
        // "not run"; it is deliberately not one of CQRRTO's own nonzero
    ...
    long cqrrto_akb = RandLAPACK::cqrrto_linops_analytical_kb<T>(m, n, d_factor, block_size);
    ...
    for (int64_t r = 0; r < num_runs; ++r) {
        results[r].cqrrto.analytical_kb      = cqrrto_akb;
...
static int run_benchmark(int argc, char *argv[]) {
    ...
    if (argc < 11 || argc > 13) {
        ...
        std::cerr << "  d_factor         : Sketching dimension factor for CQRRTO_linop (e.g., 2.0)" << std::endl;
        std::cerr << "  sketch_nnz       : (Optional) Nonzeros per column in SASO sketch (default: 4)" << std::endl;
        std::cerr << "  block_size       : (Optional) Column-block size for CQRRTO_linop/CholQR/sCholQR3 Gram (0 = full, default: 256 = paper b)" << std::endl;
        std::cerr << "\nExample:" << std::endl;
    ...
    std::cout << "\n=== CQRRTO_linop vs CholQR vs sCholQR3 vs CQRRT_expl Scaling Study ===\n";
    ...
    std::cout << "d_factor (CQRRTO_linop): " << std::fixed << std::setprecision(2) << d_factor << "\n";
    std::cout << "Sketch nnz (CQRRTO_linop): " << sketch_nnz << "\n";
    std::cout << "Block size (CQRRTO_linop, CholQR, sCholQR3): " << block_size << " (0 = full)\n";
...
static void write_results_to_csv(
    ...
    for (int64_t run = 0; run < num_runs; ++run) {
        ...
        const alg_quality<T>* algos[] = {&result.cqrrto, &result.cholqr, &result.scholqr3, &result.dense_cqrrt};
        ...
        std::vector<long> cqrrto_bd    = fit_breakdown(result.cqrrto.breakdown, 10, "CQRRTO_linop");
        ...
        for (const auto& t : cqrrto_bd)    breakdown << t << ",";
        breakdown << result.cqrrto.time << ",";
        ...
        breakdown << result.dense_cqrrt.time << ","
                  << result.cqrrto.peak_rss_kb << "," << result.cqrrto.analytical_kb << ","
                  ...
                  << result.cqrrto.qr_status << "," << result.cholqr.qr_status << ","
...
static void print_console_summary(
    ...
    int64_t best_cqrrto = -1, best_cholqr = -1, best_scholqr3 = -1, best_dense = -1;
    for (int64_t r = 0; r < num_runs; ++r) {
        if (all_runs[r].cqrrto.qr_status == 0 &&
            (best_cqrrto < 0 || all_runs[r].cqrrto.time < all_runs[best_cqrrto].cqrrto.time)) best_cqrrto = r;
    ...
    if (best_cqrrto < 0) print_alg("CQRRTO_linop", -1, T(0), 0, 0L);
    else print_alg("CQRRTO_linop", best_cqrrto, all_runs[best_cqrrto].cqrrto.orth_error, all_runs[best_cqrrto].cqrrto.max_orth_cols, all_runs[best_cqrrto].cqrrto.time);
    ...
    long cqrrto_rss    = best_cqrrto    < 0 ? 0L : all_runs[best_cqrrto].cqrrto.peak_rss_kb;
    long cqrrto_akb    = best_cqrrto    < 0 ? 0L : all_runs[best_cqrrto].cqrrto.analytical_kb;
    ...
    std::cout << "    CQRRTO_linop: " << mem_str(best_cqrrto, cqrrto_rss, cqrrto_akb)
...
// host, timestamp, and the env knobs that change sCholQR3/CQRRTO numerics
...
static void write_csv_headers(
    ...
    out << "# CQRRTO_linop vs CholQR vs sCholQR3 vs CQRRT_expl Results\n";
    out << "# Date: " << run_ts << "\n";
    out << "# host: " << host << "\n";
    out << "# argv: " << argv_line << "\n";
    out << "# Precision: " << precision << "\n";
    if (!extra_comment.empty()) out << extra_comment;
    out << "# d_factor (CQRRTO_linop only): " << d_factor << "\n";
    out << "# sketch_nnz (CQRRTO_linop only): " << sketch_nnz << "\n";
    out << "# block_size (CQRRTO_linop, CholQR, sCholQR3): " << block_size << " (0 = full)\n";
    ...
    out << "m,n,run,aspect_ratio,cond_num,density,"
        << "cqrrto_orth_error,cqrrto_max_orth_cols,cqrrto_is_orth,cqrrto_time_us,"
        ...
        << "cqrrto_peak_rss_kb,cqrrto_analytical_kb,"
        ...
        << "cqrrto_qr_status,cholqr_qr_status,scholqr3_qr_status,dense_cqrrt_qr_status,"
        << "cqrrto_chol_retries,cqrrto_chol_shift_abs,cqrrto_chol_shift_rel,"
    ...
    breakdown << "# d_factor (CQRRTO_linop only): " << d_factor << "\n";
    breakdown << "# sketch_nnz (CQRRTO_linop only): " << sketch_nnz << "\n";
    breakdown << "# block_size (CQRRTO_linop, CholQR, sCholQR3): " << block_size << " (0 = full)\n";
    ...
    // reads stale: cqrrto_trtri really holds the configured precond_method's
    // inversion time (this benchmark always runs TRSM_IDENTITY, never TRTRI);
    // cqrrto_linop_precond/cqrrto_linop_gram really hold the Gram-build's fwd/adj
    // operator applies; cqrrto_trmm_gram really holds the Gram-forming GEMM
    ...
    breakdown << "# CQRRTO_linop: alloc, saso, qr, precond_inv, fwd(gram), adj(gram), gemm(gram combine), chol(potrf), finalize, rest, total\n";
    ...
    breakdown << "m,n,run,"
              << "cqrrto_alloc,cqrrto_saso,cqrrto_qr,cqrrto_trtri,cqrrto_linop_precond,cqrrto_linop_gram,cqrrto_trmm_gram,cqrrto_potrf,cqrrto_finalize,cqrrto_rest,cqrrto_total,"
...
static int run_benchmark_from_file(int argc, char *argv[]) {
    ...
    std::cout << "\n=== CQRRTO_linop vs CholQR vs sCholQR3 vs CQRRT_expl (File Input) ===\n";
    std::cout << "Precision: " << precision.c_str() << "\n";
