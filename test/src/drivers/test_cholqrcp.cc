...
class TestCholQRCP : public ::testing::Test
{
    ...
    template <typename T>
    static void test_CholQRCP1_bench_qual(int64_t m, int64_t n, int64_t k, int64_t d, int64_t nnz, T tol, std::tuple<int, T, bool> mat_type, uint32_t seed) {
...
{ 
    for (uint32_t seed : {2})//, 1, 2})
    {
        ...
        test_CholQRCP1_bench_qual<double>(13107, 200, 200, 200, 4, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2, false), seed);

        // Tests copying sizes used in benchmarks
        //test_CholQRCP1_bench_qual<double>(5, 4, 4, 4, 4, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2, false), seed);
        //test_CholQRCP1_bench_qual<double>(131072, 2000, 2000, 2000, 4, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2, false), seed);
        //test_CholQRCP1_bench_qual<double>(131072, 2000, 2000, 4000, 4, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2, false), seed);
        //test_CholQRCP1_bench_qual<double>(131072, 2000, 2000, 6000, 4, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2, false), seed);
        //test_CholQRCP1_bench_qual<double>(131072, 2000, 2000, 20000, 4, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2, false), seed);
        //test_CholQRCP1_bench_qual<double>(131072, 5000, 5000, 20000, 4, std::pow(1.0e-16, 0.75), std::make_tuple(0, 2, false), seed);
    }
}
