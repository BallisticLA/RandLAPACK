...
protected:
    ...
    static vector<T> compute_dense_times_chol(
        ...
        int64_t rows_dense,
        int64_t dim_chol,
        RandLAPACK_extras::CholSolverLinOp<T>& chol_op
    ) {
    ...
    static void test_chol_dense(
        ...
        RandLAPACK_extras::generate_spd_matrix_file<T>(spd_file, dim_chol, 10.0, state);

        // Create CholSolver operator
        RandLAPACK_extras::CholSolverLinOp<T> chol_op(spd_file);
        chol_op.factorize();
