...
**Problem:** Script doesn't detect GPU even though CUDA is installed.

**Solution:** Ensure:
1. CUDA toolkit is properly installed
2. `nvcc` is in your PATH: `which nvcc`
3. CUDA environment variables are set (run `source ~/.bashrc`)

### Compilation Errors with GCC 13.3

**Problem:** Build fails with errors about Intel AMX intrinsics.

**Solution:** You're likely using CUDA 12.4.x. Upgrade to CUDA 12.9.0:
```shell
spack unload cuda
spack install cuda@12.9.0
...
**Problem:** Tests fail with "cannot open shared object file" errors.

**Solution:** CUDA libraries aren't in your library path. Add to `~/.bashrc`:
```shell
...
If installation fails partway through, clean up and restart:

```shell
cd ~/RandNLA
rm -rf RandNLA-project/  # Remove partial installation
cd RandLAPACK
yes | bash install.sh 2>&1 | tee fresh_install.log
```
