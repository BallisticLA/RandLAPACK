...
if __name__ == '__main__':
    ...
    for t in paths:
        p = t[0]
        residuals_amd, xaxis = parsed_residuals(t)
        residuals_nat, _     = parsed_residuals((t[0].replace('amd_true','amd_false'), t[1]))

        fig, (ax1, ax3) = plt.subplots(1, 2, figsize=(8.5, 3), sharex=True, constrained_layout=True)
        ax1.set_title('natural order')
        ax3.set_title('AMD order')
        ...
        for i, ax in enumerate([ax1, ax3]):
            if i == 0:
                residuals = residuals_nat
            else:
                residuals = residuals_amd
            lines = []
            for precname in precnames:
                resids = residuals[precname + ' preconditioning']
                arr = np.array([a[loss2loc['CG']] for a in resids])
        ...
        print('Saving figure.')
        plt.savefig(p + f'/../{lsn}-CG-amd-nat.pdf')
        continue
