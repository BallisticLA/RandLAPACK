// Test suite for CholQR_linops driver with linear operators (dense matrices)
...
TEST_F(TestDmCholQRLinops, composite_operator) {
    int64_t n_spd = 50;
    ...
    // Generate SPD matrix
    std::string spd_filename = "/tmp/test_spd_matrix_cholqr.mtx";
...
TEST_F(TestDmCholQRLinops, nested_composite_operator) {
    int64_t m = 100;
    int64_t k_dim = 50;
    int64_t n = 20;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    std::string spd_filename = "/tmp/test_cholqr_nested_small.mtx";
...
TEST_F(TestDmCholQRLinops, nested_composite_operator_large_square) {
    int64_t m = 1138;
    int64_t k_dim = 1138;
    int64_t n = 100;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    // Generate SPD matrix for CholSolver
    std::string spd_filename = "/tmp/test_cholqr_nested_large.mtx";
    RandLAPACK_demos::generate_spd_matrix_file<double>(spd_filename, m, 10.0, state);
