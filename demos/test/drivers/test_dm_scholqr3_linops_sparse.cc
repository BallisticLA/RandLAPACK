// Test suite for sCholQR3_linops driver with SPARSE SPD matrices
// This test file specifically tests the case where the SPD matrix in CholSolverLinOp is SPARSE
// Companion to test_dm_scholqr3_linops_dense.cc which tests with dense SPD matrices
...
class TestDmSCholQR3LinopsSparse : public ::testing::Test {
protected:
    virtual void SetUp() {}
    virtual void TearDown() {}
};

// Helper function to generate a sparse SPD matrix using banded structure
// Uses diagonal regularization to achieve target condition number while preserving sparsity
std::string generate_sparse_spd_matrix_scholqr3(int64_t n, double cond_num, const std::string& filename) {
...
TEST_F(TestDmSCholQR3LinopsSparse, sparse_matrix_only) {
    int64_t m = 100;
    int64_t n = 50;
...
TEST_F(TestDmSCholQR3LinopsSparse, composite_operator_sparse_spd) {
    int64_t n_spd = 50;
    ...
    std::string spd_filename = "/tmp/test_scholqr3_sparse_spd_composite.mtx";
    generate_sparse_spd_matrix_scholqr3(n_spd, 10.0, spd_filename);
...
TEST_F(TestDmSCholQR3LinopsSparse, nested_composite_operator_large_square_sparse_spd) {
    int64_t m = 1138;
    int64_t k_dim = 1138;
    int64_t n = 100;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    std::string spd_filename = "/tmp/test_scholqr3_sparse_nested_large.mtx";
    generate_sparse_spd_matrix_scholqr3(m, 10.0, spd_filename);
