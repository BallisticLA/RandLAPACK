// Unified test suite for sCholQR3_linops driver with linear operators
// Shifted Cholesky QR3: shift + 3 iterations of Cholesky QR for improved orthogonality
// Tests sCholQR3 with both dense and sparse SPD matrices in CholSolverLinOp
// Unified from test_dm_scholqr3_linops_dense.cc and test_dm_scholqr3_linops_sparse.cc
...
using RandLAPACK::sCholQR3_linops; // Algorithm now in main RandLAPACK

class TestDmSCholQR3Linops : public ::testing::Test {
protected:
    virtual void SetUp() {}
...
TEST_F(TestDmSCholQR3Linops, dense_matrix) {
    int64_t m = 100;
    int64_t n = 50;
    ...
    // Run sCholQR3 with test mode enabled
    std::vector<double> R(n * n, 0.0);
    sCholQR3_linops<double> sCholQR3(false, tol, true);
    sCholQR3.call(A_linop, R.data(), n);

    // Verify A = Q * R
    std::vector<double> QR(m * n, 0.0);
    blas::gemm(Layout::ColMajor, Op::NoTrans, Op::NoTrans, m, n, n,
               1.0, sCholQR3.Q, m, R.data(), n, 0.0, QR.data(), m);
    ...
    blas::syrk(Layout::ColMajor, Uplo::Upper, Op::Trans, n, m, 1.0, sCholQR3.Q, m, -1.0, I_ref.data(), n);
...
TEST_F(TestDmSCholQR3Linops, composite_operator_dense_spd) {
    int64_t n_spd = 50;
    ...
    std::string spd_filename = "/tmp/test_spd_matrix_scholqr3.mtx";
    ...
    // Run sCholQR3
    std::vector<double> R(n * n, 0.0);
    sCholQR3_linops<double> sCholQR3_alg(false, tol, true);
    sCholQR3_alg.call(A_composite, R.data(), n);

    // Verify A = Q * R
    std::vector<double> QR(m * n, 0.0);
    blas::gemm(Layout::ColMajor, Op::NoTrans, Op::NoTrans, m, n, n,
               1.0, sCholQR3_alg.Q, m, R.data(), n, 0.0, QR.data(), m);
    ...
    std::vector<double> I_ref(n * n);
    RandLAPACK::util::eye(n, n, I_ref.data());
    blas::syrk(Layout::ColMajor, Uplo::Upper, Op::Trans, n, m, 1.0, sCholQR3_alg.Q, m, -1.0, I_ref.data(), n);
    double norm_orth = lapack::lansy(Norm::Fro, Uplo::Upper, n, I_ref.data(), n);
...
TEST_F(TestDmSCholQR3Linops, nested_composite_operator_dense_spd) {
    int64_t m = 100;
    int64_t k_dim = 50;
    int64_t n = 20;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    std::string spd_filename = "/tmp/test_scholqr3_nested_small.mtx";
    ...
    blas::gemm(Layout::ColMajor, Op::NoTrans, Op::NoTrans, m, n, k_dim,
               1.0, saso_dense.data(), m, gaussian_data.data(), k_dim, 0.0, intermediate.data(), m);

    // Step 2: Apply CholSolver to intermediate
    std::vector<double> A_dense(m * n, 0.0);
    ...
    std::vector<double> R(n * n, 0.0);
    sCholQR3_linops<double> sCholQR3_alg(false, tol, true);
    sCholQR3_alg.call(nested_composite, R.data(), n);
...
// Test with nested composite operator (dense SPD, large scale)
TEST_F(TestDmSCholQR3Linops, nested_composite_operator_dense_spd_large_square) {
    int64_t m = 1138;
    int64_t k_dim = 1138;
    int64_t n = 100;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    // Generate dense SPD matrix for CholSolver
    std::string spd_filename = "/tmp/test_scholqr3_nested_large.mtx";
    RandLAPACK_demos::generate_spd_matrix_file<double>(spd_filename, m, 10.0, state);
    ...
    // Run sCholQR3 (no dense verification for large scale)
    std::vector<double> R(n * n, 0.0);
    sCholQR3_linops<double> sCholQR3_alg(false, tol, true);
    sCholQR3_alg.call(nested_composite, R.data(), n);

    // We can't verify against dense representation due to size
    // Instead, verify orthogonality of Q and that R is upper triangular
    std::vector<double> I_ref(n * n);
    RandLAPACK::util::eye(n, n, I_ref.data());
    blas::syrk(Layout::ColMajor, Uplo::Upper, Op::Trans, n, m, 1.0, sCholQR3_alg.Q, m, -1.0, I_ref.data(), n);
    double norm_orth = lapack::lansy(Norm::Fro, Uplo::Upper, n, I_ref.data(), n);
...
TEST_F(TestDmSCholQR3Linops, sparse_matrix_only) {
    int64_t m = 100;
    int64_t n = 50;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    auto state = RandBLAS::RNGState();
...
TEST_F(TestDmSCholQR3Linops, composite_operator_sparse_spd) {
    int64_t n_spd = 50;
    int64_t n_sparse_cols = 20;
    int64_t m = n_spd;
    int64_t n = n_sparse_cols;
    ...
    std::string spd_filename = "/tmp/test_sparse_spd_matrix_scholqr3.mtx";
...
TEST_F(TestDmSCholQR3Linops, nested_composite_operator_sparse_spd_large_square) {
    int64_t m = 1138;
    int64_t k_dim = 1138;
    int64_t n = 100;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    std::string spd_filename = "/tmp/test_scholqr3_nested_large_sparse_spd.mtx";
