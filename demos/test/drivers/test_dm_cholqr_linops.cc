// Unified test suite for CholQR_linops driver with linear operators
// Tests both dense and sparse SPD matrices in CholSolverLinOp
//
// This file unifies test_dm_cholqr_linops_dense.cc and test_dm_cholqr_linops_sparse.cc
// to reduce code duplication while maintaining comprehensive test coverage
...
using RandLAPACK::CholQR_linops;   // Algorithm now in main RandLAPACK

class TestDmCholQRLinops : public ::testing::Test {
protected:
    virtual void SetUp() {}
...
// ============================================================================
// Tests with Dense SPD Matrices
// ============================================================================

// Test with simple dense matrix wrapped in DenseLinOp
TEST_F(TestDmCholQRLinops, dense_matrix) {
    int64_t m = 100;
    int64_t n = 50;
    ...
    // Run CholQR with test mode enabled
...
// Test with composite operator: CholSolver * Sparse (dense SPD)
TEST_F(TestDmCholQRLinops, composite_operator_dense_spd) {
    int64_t n_spd = 50;
    ...
    std::string spd_filename = "/tmp/test_spd_matrix_cholqr_dense.mtx";
...
// Test with nested composite operator: CholSolver * (SASO * Gaussian) (dense SPD, small scale)
TEST_F(TestDmCholQRLinops, nested_composite_operator_dense_spd) {
    int64_t m = 100;
    int64_t k_dim = 50;
    int64_t n = 20;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    std::string spd_filename = "/tmp/test_cholqr_nested_dense_small.mtx";
...
// Test with nested composite operator: CholSolver * (SASO * Gaussian) (dense SPD, large scale)
TEST_F(TestDmCholQRLinops, nested_composite_operator_dense_spd_large_square) {
    int64_t m = 1138;
    int64_t k_dim = 1138;
    int64_t n = 100;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
    ...
    // Generate dense SPD matrix for CholSolver
    std::string spd_filename = "/tmp/test_cholqr_nested_dense_large.mtx";
    RandLAPACK_demos::generate_spd_matrix_file<double>(spd_filename, m, 10.0, state);
...
TEST_F(TestDmCholQRLinops, composite_operator_sparse_spd) {
    int64_t n_spd = 50;
    int64_t n_sparse_cols = 20;
    int64_t m = n_spd;
    int64_t n = n_sparse_cols;
...
TEST_F(TestDmCholQRLinops, nested_composite_operator_sparse_spd_large_square) {
    int64_t m = 1138;
    int64_t k_dim = 1138;
    int64_t n = 100;
    double tol = std::pow(std::numeric_limits<double>::epsilon(), 0.85);
