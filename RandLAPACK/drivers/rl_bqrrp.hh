    ...
    for(iter = 0; iter < maxiter; ++iter) {
        ...
        if((curr_sz >= std::min(m, n)) || (block_rank != b_sz_const)) {
            ...
            if(this -> timing) {
                ...
                std::cout << "SKOP generation and application takes     " << std::fixed << std::setw(6) << std::setprecision(2) << 100 * ((T) skop_t_dur                / (T) total_t_dur) << "% of runtime.\n";
                std::cout << "QRCP_wide takes                           " << std::fixed << std::setw(6) << std::setprecision(2) << 100 * ((T) qrcp_wide_t_dur                / (T) total_t_dur) << "% of runtime.\n";
                std::cout << "Panel preprocessing takes                 " << std::fixed << std::setw(6) << std::setprecision(2) << 100 * ((T) panel_preprocessing_t_dur / (T) total_t_dur) << "% of runtime.\n";
                ...
                std::cout << "Everything else takes                     " << std::fixed << std::setw(6) << std::setprecision(2) << 100 * ((T) t_other                   / (T) total_t_dur) << "% of runtime.\n";
